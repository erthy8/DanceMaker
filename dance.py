...
if __name__ == '__main__':
    ...
    with mp.hands.Hands(static_image_mode=False, model_complexity=1, max_num_hands=1, min_detection_confidence=0.7,
        ...
        # DrawingSpec(color=(B, G, R))
        DrawingSpec: object = mp.drawing_utils.DrawingSpec
        ...
        bbx_offset_x, bbx_offset_y = round(
            (1 - bbx_scale) / 2, 2), round((1 - bbx_scale) / 2, 2)  # begin in center
        bounding_box = (
        ...
        while True:  # cap.isOpened():
            ...
            if results.multi_hand_landmarks:
                for hand_landmarks in results.multi_hand_landmarks:
                    ...
                    landmark_list: List[List[float]] = np.array(
                        [[lmd.x, lmd.y, lmd.z] for lmd in hand_landmarks.landmark])

                    # calculate position relative to bounding box
                    pointer_abs_pos: List[float] = landmark_list[key_lmd['index_base']]
                    uncapped_rel_pos = (
                        round((pointer_abs_pos[0] - bounding_box[0][0]) /
                    ...
                    thumb_angle: float = get_angle(
                        landmark_list, key_lmd['thumb_click'])
                    index_angle: float = get_angle(
                        landmark_list, key_lmd['index_click'])
                    middle_angle: float = get_angle(
                        landmark_list, key_lmd['middle_click'])
                    ring_angle: float = get_angle(
                        landmark_list, key_lmd['ring_click'])
                    pinky_angle: float = get_angle(
                        landmark_list, key_lmd['pinky_click'])
                    ...
                    if config['move'] and not locked:
                        ...
                        new_pos = scale_coords(
                            point=capped_rel_pos, dims=disp_dim)
                        ...
                        for p in bounding_box:
                            p[0] += x_tran if 0 < bounding_box[0][0] + \
                                x_tran and bounding_box[1][0] + x_tran < 1 else 0
                            p[1] += y_tran if 0 < bounding_box[0][1] + \
                                y_tran and bounding_box[1][1] + y_tran < 1 else 0

                        if abs(new_pos[0] - mouse_pos[0] + new_pos[1] - mouse_pos[1]) >= move_distance_threshold:
                            c1 = mouse_pos[0] + \
                                (new_pos[0] - mouse_pos[0]) / smoothing
                            c2 = mouse_pos[1] + \
                                (new_pos[1] - mouse_pos[1]) / smoothing
                    ...
                    if config['display']:
                        ...
                        mp_drawing.draw_landmarks(image, hand_landmarks, [(1, 2), (2, 3), (3, 4)],
                                                  ...
                                                  connection_drawing_spec=DrawingSpec(
                                                      color=(0, 255, 0) if thumb_active else (
                                                          211, 0, 148),
                                                      thickness=2))
                        ...
                        mp_drawing.draw_landmarks(image, hand_landmarks, [(17, 18), (18, 19), (19, 20)],
                                                  landmark_drawing_spec=DrawingSpec(color=(255, 255, 255), thickness=2,
                                                                                    circle_radius=1),
                                                  connection_drawing_spec=DrawingSpec(
                                                      color=(0, 255, 0) if pinky_active else (
                                                          211, 0, 148),
                                                      thickness=2))
                        ...
                        web_bbx = tuple(map(lambda coord: scale_coords(
                            point=coord, dims=cap_dim), bounding_box))
                        cv2.rectangle(image, web_bbx[0], web_bbx[1], color=(
                            255, 255, 0), thickness=2)

                        # display  position, left angle, right angle
                        cv2.putText(
                            image, f'{capped_rel_pos}', (0, 40), cv2.FONT_HERSHEY_PLAIN, 2, (255, 255, 0), 2)
            ...
            if config['display']:
                ...
                pTime = cTime
                cv2.putText(
                    image, f'{int(fps)}', (cap_dim[0] - 40, 40), cv2.FONT_HERSHEY_PLAIN, 2, (255, 255, 255), 2)
                cv2.namedWindow('MediaPipe Hands', cv2.WINDOW_AUTOSIZE)
                if config['absolute_display']:
                    cv2.setWindowProperty(
                        'MediaPipe Hands', cv2.WND_PROP_TOPMOST, 1)
