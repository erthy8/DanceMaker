...
def show_dance_message():
    ...
    ready_label = tk.Label(root, text=f"Score: {score}", bg="#2E2E2E", fg="white", font=comic_sans_font)
    ready_label.pack(pady=(window_height-70)//2)  # Center vertically
    ready_label.pack_configure(anchor="center")

    ready_button = ttk.Button(root, text="Ready", command=live_processing(), style="RoundedPink.TButton")
...
# Create a label
label = tk.Label(root, text="Enter a YouTube URL or select an HDF5 file:", bg="#2E2E2E", fg="white", font=comic_sans_font)
label.pack(pady=10)
...
# Create a label
label = tk.Label(root2, text=f"Score = {round((score*100), 2)}", bg="#2E2E2E", fg="white", font=comic_sans_font)
label.pack(pady=10)
